...
Ext.extend(MODx.grid.SettingsGrid,MODx.grid.Grid,{
    ...
    ,clearFilter: function() {
        var ns = MODx.request['ns'] ? MODx.request['ns'] : 'userprofile2';
    ...
    ,filterByKey: function(tf,newValue,oldValue) {
        ...
        this.getStore().baseParams.namespace = 'userprofile2';
        this.getBottomToolbar().changePage(1);
        this.refresh();
        return true;
    }
    ...
    ,filterByNamespace: function(cb,rec,ri) {
        ...
        if (acb) {
            var s = acb.store;
            s.baseParams['namespace'] = 'userprofile2';
            s.removeAll();
            s.load();
            acb.setValue('');
        }
...
Ext.reg('userprofile2-grid-setting',MODx.grid.SettingsGrid);
