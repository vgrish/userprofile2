...
if (!empty($_REQUEST['type']) && in_array($_REQUEST['type'], $allowedType)) {
...
if (!empty($row['type']) && $TabsFields = $userprofile2->getTabsFields($row['type'])) {
    ...
    foreach ($TabsFields as $tabName => $tab) {
        ...
        foreach ($tab['fields'] as $fieldName => $field) {
            ...
            if ($row['type_out'] == 'textarea') {
                $row['fieldrows'] .= empty($tplContentTabPaneTextareaRow)
                    ? $userprofile2->pdoTools->getChunk('', $row)
                    : $userprofile2->pdoTools->getChunk($tplContentTabPaneTextareaRow, $row,
                        $userprofile2->pdoTools->config['fastMode']);
            } else {
                $row['fieldrows'] .= empty($tplContentTabPaneInputRow)
                    ? $userprofile2->pdoTools->getChunk('', $row)
                    : $userprofile2->pdoTools->getChunk($tplContentTabPaneInputRow, $row,
                        $userprofile2->pdoTools->config['fastMode']);
            }
        }
