...
if (empty($processSection)) {
    $processSection = 'tickets,comments,favorites';
}
if (empty($pleTickets)) {
    $pleTickets = 'tickets';
}
if (empty($pleComments)) {
    $pleComments = 'comments';
}
if (empty($pleFavorites)) {
    $pleFavorites = 'favorites';
}
//
    ...
    if (!empty($parents)) {
        ...
        foreach ($pids as $v) {
            $parents = array_merge($parents,
                $modx->getChildIds($v, $depth, array('context_key' => $modx->context->key)));
        }
    }
}
if (!empty($processTickets)) {
    // Tickets
    $where = array('createdby'  => $user_id,
                   'deleted'    => 0,
                   'published'  => 1,
                   'class_key'  => 'Ticket',
                   'privateweb' => 0
...
foreach ($count as $k => $c) {
    if (!empty($toPlaceholders)) {
        ...
        $output[] = empty($tplCount)
            ...
            : $userprofile2->pdoTools->getChunk($tplCount, array('count' => $c, 'name' => $k),
...
$output = empty($tplCounts)
    ...
    : $userprofile2->pdoTools->getChunk($tplCounts, array('counts' => $output),
        $userprofile2->pdoTools->config['fastMode']);
