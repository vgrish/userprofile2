...
if ($object->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            $modelPath = $modx->getOption('virtualpage_core_path', null,
                    $modx->getOption('core_path') . 'components/virtualpage/') . 'model/';
            ...
            foreach ($events as $id => $properties) {
                if (!$event = $modx->getObject('vpEvent', array('name' => $properties['name']))) {
                    $event = $modx->newObject('vpEvent', array_merge(array(//'name' => $properties['name'],
                    ), $properties));
                }
                $event->set('active', 1);
                $event->save();
            ...
            foreach ($routes as $id => $properties) {
                if (!$route = $modx->getObject('vpRoute', array('route' => $properties['route']))) {
                    $route = $modx->newObject('vpRoute', array_merge(array(//'name' => $properties['name'],
                    ), $properties));
                    $route->set('active', 1);
                    $route->save();
                }
